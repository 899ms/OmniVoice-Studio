...
render as the identical floor message, because `classify()` either has no topic
...
def test_an_unsupported_gpu_is_named_instead_of_the_floor_message():
    payload = stream_generation_failure(RuntimeError(NO_KERNEL_IMAGE))
    ...
    assert "cu128" in payload["hint"]
    # The floor message is still the lead; the hint is appended to it.
    assert payload["detail"].startswith(stream_failure("generation_failed")["detail"])
    assert payload["detail"] != stream_failure("generation_failed")["detail"]


def test_an_unsupported_gpu_is_terminal_not_an_invitation_to_retry():
...
def test_an_os_blocked_load_reaches_the_client():
    ...
    payload = stream_generation_failure(RuntimeError(APP_CONTROL_BLOCKED))
...
def test_an_audio_io_failure_reaches_the_client():
    ...
    payload = stream_generation_failure(RuntimeError(AUDIO_IO))
...
def test_an_unclassified_failure_is_byte_identical_to_before():
    payload = stream_generation_failure(RuntimeError("Something went sideways"))
...
def test_out_of_memory_is_untouched():
    """The new branch sits after the OOM check, so real memory pressure keeps
    its own class and stays retryable."""
    payload = stream_generation_failure(RuntimeError("CUDA out of memory. Tried to allocate 2.00 GiB"))
...
def test_a_generic_failure_still_gets_no_hint(message):
...
def test_the_new_class_classifies_on_its_own():
    assert classify(NO_KERNEL_IMAGE) == "GPU_ARCH_UNSUPPORTED"
...
def test_every_terminal_class_can_actually_reach_the_client():
    ...
    assert _TERMINAL_FAILURE_CLASSES <= _CONTEXT_FREE_HINT_CLASSES

...
describe('WinINET proxy normalization', () => {
  ...
  it('routes per-protocol proxies without changing the tunnel protocol', () => {
    expect(parseWinInetProxy('http=one:80;https=two:81;socks=three:1080')).toEqual({
      HTTP_PROXY: 'http://one:80', HTTPS_PROXY: 'http://two:81',
    });
    expect(parseWinInetProxy('https://secure:8443')).toEqual({
      HTTP_PROXY: 'https://secure:8443', HTTPS_PROXY: 'https://secure:8443',
    });
  });
  ...
  it('preserves explicit proxy choices and loopback exclusions', () => {
    ...
    const env = downloadProxyEnv({ https_proxy: 'http://explicit:80', NO_PROXY: 'internal' }, 'win32', read);
  ...
  it('reads Windows settings only and exposes only proxy variables', () => {
    ...
    expect(downloadProxyEnv({ PYTHONPATH: '/untrusted', CUSTOM: 'keep' }, 'linux', read).PYTHONPATH).toBeUndefined();
    read.mockClear();
...
it('preserves representable system bypasses and declines lossy conversions', () => {
  expect(proxyBypassHosts('*.example.com;internal;127.0.0.1')).toEqual(['.example.com', 'internal', '127.0.0.1']);
  const env = downloadProxyEnv({}, 'win32', () => ({server: 'socks=proxy:1080', bypass: '*.example.com'}));
  expect(env.NO_PROXY).toContain('.example.com');
  expect(downloadProxyEnv({}, 'win32', () => ({server: 'socks=proxy:1080', bypass: '<local>'})).HTTPS_PROXY).toBeUndefined();
});


it('reads enabled registry values with a timeout and handles denied access', () => {
  vi.mocked(execFileSync).mockReturnValue('ProxyEnable    REG_DWORD    0x1\nProxyServer    REG_SZ    socks=127.0.0.1:10808\nProxyOverride    REG_SZ    *.internal.example');
  expect(downloadProxyEnv({}, 'win32').HTTPS_PROXY).toBe('socks5h://127.0.0.1:10808');
  expect(execFileSync).toHaveBeenCalledWith('reg.exe', expect.any(Array), expect.objectContaining({ timeout: 2500, windowsHide: true }));
  vi.mocked(execFileSync).mockReturnValue('ProxyEnable    REG_DWORD    0x0\nProxyServer    REG_SZ    socks=127.0.0.1:10808');

...
export function classifySherpaFinal(message: { text?: string; final_kind?: string }): 'summary' | 'terminator' | 'utterance' | 'ignore';
...
export function computeTypeDelta(previous: string, next: string): { backspaces: number; text: string; noop: boolean };

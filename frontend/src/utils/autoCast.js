...
export function buildAutoCast(text, cast = [], profiles = []) {
  ...
  for (const name of speakers) {
    let character = next.find(c => (name.toLowerCase() === 'narrator' && c.id === 'narrator') || c.name.toLowerCase() === name.toLowerCase());
    if (!character) {
      const base = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'char';
  ...
  return { cast: next, tracks: parsed.map(line => ({ character: ids.get(line.speaker), text: line.text })), speakers };
}

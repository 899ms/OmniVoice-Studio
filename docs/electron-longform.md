...
Stories imports TXT, Markdown and SRT locally; EPUB/PDF still use the shared backend importer. The EPUB importer skips marked pagination and ancillary sections such as cover, title, dedication, contents and copyright pages. Unmarked pages are judged by the book's own structure: when the book has a table of contents, an unlisted page ahead of the declared reading start (EPUB 2 guide `text` reference or EPUB 3 landmarks `bodymatter`) is skipped; without a declared start — or without a table of contents to cross-check it — only a short, heading-less, unlisted page ahead of the start is skipped; the same kind of stray page is skipped at the back only once back matter that the contents lists has begun, so an unlisted file that continues the last chapter is always kept. Unmarked numeric paragraphs and page-break layout classes remain narration. Chapter titles prefer the EPUB 3 table of contents, then NCX labels and headings; namespace aliases and declared encodings are supported. Malformed optional navigation falls back to headings, while malformed package/container XML reports an import error. Imported text goes into a persisted review buffer, then Auto-cast or Split into lines appends to existing work. The sentence-aware splitter is shared with the browser app and offers the same 40-2000 character limit.

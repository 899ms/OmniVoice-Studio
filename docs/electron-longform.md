...
Stories imports TXT, Markdown and SRT locally; EPUB/PDF still use the shared backend importer. The EPUB importer narrates only the book's body: print page numbers are dropped, cover, title, dedication, contents and copyright pages are skipped, and each chapter takes its title from the book's table of contents. Imported text goes into a persisted review buffer, then Auto-cast or Split into lines appends to existing work. The sentence-aware splitter is shared with Tauri and offers the same 40-2000 character limit.
